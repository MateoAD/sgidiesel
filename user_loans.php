...
<body class="bg-gray-50">
                ...
                <div x-show="open" class="fixed top-0 right-0 h-full w-64 bg-gray-50 shadow-xl z-50 transform transition-transform duration-300 ease-in-out"
                    ...
                    <div class="p-4 h-full flex flex-col">
                        ...
                        <div class="flex-grow overflow-y-auto space-y-2">
                            ...
                            <a href="user_inventory.php" class="block px-4 py-3 text-gray-700 hover:bg-gray-200 hover:text-[#4A655D] rounded-lg transition duration-200">
    ...
    <main class="container mx-auto px-4 py-8 max-w-6xl">
        <div
            ...
            <h1 class="text-2xl font-bold text-gray-800 mb-8">Agregar Préstamo</h1>
                ...
                <div class="bg-white rounded-lg shadow-sm p-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea id="descripcion-prestamo" class="polymorphic-input w-full px-4 py-2"
                                placeholder="Detalles adicionales del préstamo" rows="3"></textarea>
                        </div>
                    </div>
                </div>
...
<div class="polymorphic-container bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-2xl shadow-lg mt-8">
    ...
    <div class="table-container max-h-96 overflow-y-auto">
        ...
        $queryReservas = "SELECT 
            ...
            r.cantidad,
            CASE 
                WHEN r.tipo_herramienta = 'consumible' THEN hc.nombre
                ELSE hnc.nombre
        ...
        <?php if (empty($reservasPendientes)): ?>
            <div class="text-center py-8">
                ...
                <h3 class="text-lg font-medium text-gray-700">No hay reservas pendientes</h3>
                <p class="text-gray-500">No hay solicitudes de reserva por aprobar en este momento.</p>
            </div>
        <?php else: ?>
                ...
                <tbody class="divide-y divide-gray-200">
                    <?php foreach ($reservasPendientes as $reserva): ?>
                        <tr class="hover:bg-blue-50">
                            ...
                            <td class="px-6 py-4">
                                <?= htmlspecialchars($reserva['nombre_herramienta']) ?>
                                <div class="text-sm text-gray-500">
                                    <?= $reserva['tipo_herramienta'] === 'consumible' ? 'Consumible' : 'No Consumible' ?>
                                </div>
                            </td>
                            <td class="px-6 py-4"><?= htmlspecialchars($reserva['cantidad']) ?></td>
                            <td class="px-6 py-4"><?= date('d/m/Y H:i', strtotime($reserva['fecha_reserva'])) ?></td>
                            <td class="px-6 py-4">
                                <form method="post" class="flex space-x-2" onsubmit="procesarReserva(event, this)">
...
<!-- Modal para mensajes -->
<div id="reserva-modal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white rounded-lg p-6 max-w-md w-full">
        <div class="flex justify-between items-center mb-4">
            <h3 id="reserva-modal-title" class="text-xl font-bold"></h3>
            <button onclick="document.getElementById('reserva-modal').classList.add('hidden')" 
                class="text-gray-500 hover:text-gray-700">
                <i class="fas fa-times"></i>
            </button>
        </div>
        <p id="reserva-modal-message"></p>
        <div class="mt-4 flex justify-end">
            <button onclick="document.getElementById('reserva-modal').classList.add('hidden'); window.location.reload();" 
    ...
    if (window.location.protocol !== 'https:' && window.location.hostname !== 'localhost') {
        mostrarMensaje('La cámara solo funciona en conexiones HTTPS seguras', 'error');
        return;
    }
...
async function procesarReserva(event, form) {
    ...
    if (action === 'rechazar' && !confirm('¿Está seguro de rechazar esta reserva?')) {
        return false;
    }
    ...
    try {
        ...
        if (data.error) {
            mostrarModalReserva('Error', data.message, 'error');
        } else {
            mostrarModalReserva('Éxito', data.message, 'success');
        }

    } catch (error) {
        console.error('Error:', error);
        mostrarModalReserva('Error', 'Ocurrió un error al procesar la reserva', 'error');
    }
}

// Función para mostrar modal de reserva
function mostrarModalReserva(titulo, mensaje, tipo) {
    const modal = document.getElementById('reserva-modal');
    const modalTitle = document.getElementById('reserva-modal-title');
    const modalMessage = document.getElementById('reserva-modal-message');
